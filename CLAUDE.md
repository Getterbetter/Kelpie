...
- A single suite runs with `xcodebuild test -project Heeler.xcodeproj -scheme Heeler -destination 'platform=iOS Simulator,name=iPad Air 11-inch (M4)' -only-testing:HeelerTests/<SuiteTypeName>`; `make test` runs everything on `SIM` (default iPad Air 11-inch (M4)). `make test-ipad` re-invokes that on `SIM_IPAD` (default iPad Pro 13-inch (M5)). The `Packages/HeelerSSH` package suites are a separate test plan run by `scripts/run-heelerssh-package-tests.sh` — changes under `Packages/HeelerSSH` need that runner, not `-only-testing:HeelerTests/...`.
