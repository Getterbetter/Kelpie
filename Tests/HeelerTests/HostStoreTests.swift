...
struct HostStoreTests {
    ...
    /// Kelpie reads a malformed Host as one hole with a notice (see
    /// `oneUnreadableHostDoesNotTakeTheCatalogWithIt`), where upstream refuses
    /// the catalog; either way the entry survives a later write.
    @Test func malformedKnownAuthHostIsHiddenAndPreserved() throws {
        let (defaults, cleanup) = try makeDefaults()
        defer { cleanup() }
        let catalog = Data("""
            {"version":1,"hosts":[
        ...
        #expect(try persistedHosts(in: defaults).first?["port"] as? String == "twenty-two")
        #expect(try persistedHosts(in: defaults).count == 2)
    }
