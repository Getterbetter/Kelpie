...
# Kelpie builds for iPad and iPhone; CI boots an iPad simulator. This model
# exists on the macos-26 runners and on Anthony's Mac.
ci_simulator_name="${HEELER_CI_SIM_MODEL:-${HEELER_CI_SIMULATOR_NAME:-iPad Air 11-inch (M4)}}"
...
run_suite SharedFixtureE2ETests 99 6 0 \
...
if grep -q 'Suite "Session driver resource e2e" skipped' "$package_e2e_log" \
    ...
    || ! grep -q 'Test run with 61 tests in 5 suites passed' "$package_e2e_log" \
    || ! grep -q \
    ...
    echo "The mandatory HeelerSSH package suites did not execute all sixty-one tests" >&2
    exit 1
fi
exit 0
fi
