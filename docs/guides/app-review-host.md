...
working — no Mac behind it and no reverse tunnel, just
`reviewer iPad -> VPS public TCP 22 -> herdr API socket + Attach PTY`. For the
NAT case (a Mac behind a jump host) use
[VPS Jump Host and Reverse-Tunnel Deployment](vps-jump-host.md) instead. About
20 minutes end to end; delete the VPS once the app is approved.
...
Any provider works. Choose **Ubuntu 24.04 LTS**, the smallest instance
(**1 vCPU / 1 GB RAM**, plus the default swap if offered), a **public IPv4**,
and **SSH public key at create time with password login off**. IPv6-only will
not do: the Pairing Code carries addresses the iPad must reach directly.
...
As root on the VPS (`ssh root@VPS_IP`): update, create the account, add your key.
...
Then the SSH policy as a drop-in (never an edit to `sshd_config`), the
firewall, and unattended security updates:
...
ssh-keygen -lf /etc/ssh/ssh_host_ed25519_key.pub    # HOST_FP, for the notes
```

`AllowStreamLocalForwarding yes` is load-bearing: Kelpie reaches herdr's JSON
API over OpenSSH **direct-streamlocal** channels onto the Unix socket, and a
server that denies stream-local forwarding fails preflight rather than falling
back. Do **not** set `DisableForwarding`, and leave `MaxSessions` at its default
of 10 — the RPC queue and the Attach PTY are ordinary session channels. Keep
this root session open until step 6 has passed.
...
If Kelpie is not on GitHub yet, copy the tree up from the Mac and link it
instead — `plugin link` runs no build command, so build first:
...
documented purpose, not from upstream. If it misbehaves, fall back to
`tmux new -d -s herdr herdr` with lingering still enabled. Confirm either way by
logging out, back in, and running `herdr status server`.
...
`ls`. Skip the rest of this step if no Anthropic API key is available.

With a key, add one Claude Code agent — install Claude Code per Anthropic's
current instructions, then **trust the directory first**. Starting claude in a
cwd absent from `~/.claude.json` blocks on an in-TUI trust dialog herdr cannot
dismiss, and the agent wedges before its first prompt.
...
The action opens a popup **inside herdr's own window**, not in the shell, so
attach the TUI with `herdr` first, then from the TUI or a second shell:
...
From `plugin/src/pairing-session.js` and `pair-popup.js`: `PAIRING_TTL_SECONDS
= 120`, so the code's Bootstrap Key lives **two minutes**, and it is **single
use** — the `authorized_keys` line self-revokes the moment a device enrolls,
and is removed when the TTL lapses or the popup exits. A Pairing Code pasted
into review notes is therefore dead long before a reviewer opens the app. Mint
one only when someone is watching the reviewer pair in real time, and
regenerate on any **Resubmit**. Otherwise use the fallback.
...
one account (remove this at teardown):
...
The reviewer confirms the host key fingerprint (TOFU) on first connect, so put
`HOST_FP` in the notes.

## 5. App Review notes

Paste into **App Review Information → Notes** (about 110 words):
...
> On first launch, tap "Add manually over SSH" and enter address `VPS_IP`, port
> 22, user `review`, method Password, password `REVIEW_PW`. Accept the host key
> fingerprint `HOST_FP` when asked.

Attach a **30–60 second screen recording** of the real flow on the iPad: Welcome
screen, paste or enter, the terminal appearing, `ls` echoing back. Reviewers
often never get as far as the app, so the video carries the submission.

## 6. Verify from the Mac before submitting

1. `ssh review@VPS_IP 'herdr status server && herdr workspace list'` succeeds
   with key auth and reports a running server.
2. On the iPad, **delete and reinstall Kelpie** so it starts with no Hosts and a
   fresh Device Key, then pair exactly as the notes describe. herdr's screen must
   appear and `ls` must respond in the shell pane.
3. Log the SSH session out entirely, wait, and repeat step 1 — that proves the
   lingering unit.
...
provider console, and forget its host key with `ssh-keygen -R VPS_IP`.
...
Mint the new code, or reset `REVIEW_PW`, immediately before pressing Submit.
