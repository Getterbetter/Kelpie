...
enum AgentLayoutTokensEditing {
    ...
    static func addFieldFooter(rowIndex: Int) -> String {
        ...
        case .herdr?:
            "This row starts from herdr's sidebar fields; Sync from plugin refills it. Kelpie fields are welcome here too."
        case .heeler?:
            "This is Kelpie's row. Sync from plugin fills it only when herdr defines a third row."
        case nil:
            ""
        }
    }
    ...
    static func description(for token: AgentRowToken) -> String {
        switch token {
        case .stateIcon:
            "Status icon. Kelpie's status column at the end of Row 1 always shows it; it is not offered as a field."
        case .stateText:
