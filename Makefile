...
# Kelpie: `make install` falls back to the first physical iPad when no iPhone
# is paired, so a Mac with only the iPad still installs.
DEVICE ?= $(shell python3 scripts/find-ios-device.py iPhone)
ifeq ($(strip $(DEVICE)),)
DEVICE := $(shell python3 scripts/find-ios-device.py iPad)
endif
...
# The community watch and the posting lane moved out of this repo on 2026-09-16:
# they are Anthony's local social-management tooling, not part of the app, and
# they live in ~/Developer/kelpie-social with their own launchd jobs and tests.
