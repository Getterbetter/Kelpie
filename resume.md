...
6. **TestFlight build 5** carrying the composer, the tap fix and the lease (the plugin on the mini already reads the lease). Anthony, closing round 20: "ill start a new session next after pushing to TestFlight", so this may already be done when the next session opens; check the build number in `project.yml` and App Store Connect before doing it again. Recipe: `make bump`, then the archive, export and `altool` steps under "TestFlight upload, the recipe that works" in `KelpieVault/Build and deploy.md`.
