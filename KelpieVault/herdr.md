...
On the mini, `herdr plugin install` takes `OWNER/REPO[/SUBDIR] --ref` from GitHub only (no local path); the Heeler-compatible plugin installed there is Kelpie's own since 2026-09-15 (`Getterbetter/Kelpie/plugin --ref kelpie`). `herdr` is `~/.local/bin/herdr`, version **0.9.1** (protocol 22) since Anthony's restart on 2026-09-17 14:03, 0.8.2 before it (not on a non-interactive login's `PATH`; both the app and the [[Dependency watch]] add the well-known prefixes). Kelpie's committed API schema snapshot is 0.9.1 (protocol 22) and equals the mini's live export; the load-bearing facts in `CLAUDE.md` were re-verified live against it on 2026-09-23 (round 32, `Archive/round32/herdr-0.9.1-live.md`). 0.9.1 adds a refusal (`agent.rename` → `agent_launch_pending` during a pending launch), three `ping` capabilities (`endpoint_protocol_generation`, `surface_interest`, `health_check`), and a `pane.graphics.stream` variant its parser accepts but its schema omits; the app admits older servers by a protocol floor. The watch reads the mini's version daily and flags it when it runs ahead of the snapshot.
