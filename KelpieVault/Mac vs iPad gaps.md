...
| 7 | **Mouse drag to resize panes, sidebar drag** | Trackpad drag is already reported by the vendored view (motion under `?1002h`). **Finger** tap/drag on the sidebar does not work — round 6 (see [[Feedback log]] round 5) | Encode SGR motion events while a trackpad button is held; check on the device first | M |
