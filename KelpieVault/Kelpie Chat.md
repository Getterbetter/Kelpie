...
## Paused 2026-09-16, after round 24

Anthony: "i think i want to pause this activity." One build round (43a) shipped; 43b–43f are not started. Nothing here is wired into the app: the new code is a transport method, a read model and their tests, reachable only from the test target. The app on both devices behaves exactly as before the round. This section is the pick-up path.

### What was found, in order of how much it would cost to rediscover

1. **The chat can only come from Claude Code's transcript file on the host**, never from herdr's API (finding 1 below, ADR 0019). A pane maps to that file exactly through `pane.process_info` and `~/.claude/sessions/<pid>.json`. Verified live on the mini (herdr 0.8.2, Claude Code 2.1.273).
2. **The transcript's real shape differs from the spike's digest.** Every message line carries `uuid`, `parentUuid`, `sessionId`, `cwd`, `version`, `gitBranch`, `timestamp`; one API message is written as several lines, one content block each, sharing `message.id`, with the results of parallel tool calls written *between* them; `user` lines can be `isMeta: true` (local-command echoes) and carry `permissionMode`; `system` has subtypes beyond `turn_duration` (`local_command`, `away_summary`, `scheduled_task_fire`); `agent-name`, `cost-state` and `queue-operation` lines exist; image results carry `media_type`. The format moved between 2.1.273 and 2.1.27x in the same fortnight, so the parser is lenient by type and the fixture is re-cut by script, not typed by hand.
3. **A byte-offset read over exec is cheap and correct if it is byte-exact.** `tail -c +N | head -c M` between markers, size printed *after* the body (a growing file then never reports a false end), no newline normalisation (one normalised CRLF would shift every later offset), `tail -c` is 1-based. 0.2–0.4 s per page over loopback; the Tailscale figure is unmeasured.
4. **Permission prompts are renderable** (finding 4): herdr `blocked` + a tool_use with no result + the option list read from the visible screen; `agent.send_keys` answers. Auto mode passes through `blocked` for ~5 s and answers itself, so read the `permission-mode` line first.
5. **How the UI would be tested, given this repo** (from the round-24 survey of `Tests/HeelerTests`): there is no behavioural UI-test target (`HeelerUITests` is a manual screenshot driver, skipped by `make test` and CI), no snapshot library, and no iPhone simulator lane anywhere (`SIM` and CI both default to iPad). The house style puts the assertion below the `View`: a `*Presentation`/`*Renderer` value type tested directly (`HostListViewTests`, `AgentRowRendererTests`), a `@MainActor` store driven by `ScriptedTransport` and its call gates for mid-flight state (`AgentComposerStoreTests` is the closest analogue to a chat composer), `ImageRenderer` pixel comparisons for layout budgets (`AgentActivityPresentationTests`), and `makeTestWindow` when a responder chain is needed. An iPhone-only chat therefore gets automated iPhone coverage only from `make test-device-iphone` on hardware, unless a size-class-parameterised `ImageRenderer` test or an iPhone simulator lane is added first.

### To pick it up

1. Read this note top to bottom, ADR 0019, and *What exists* below; the round-24 code is `bb522bb`, its review `Archive/round24/review-1.md`.
2. Check the transcript format has not moved further: `ls -t ~/.claude/projects/-Users-anthonytopalides-Developer-Kelpie | head`, then re-cut the fixture with `scripts/cut-transcript-fixture.py <two recent .jsonl files>` and run `ClaudeTranscriptParserTests` on a device. New line types are skipped silently; a changed *shape* of `user`/`assistant` lines is what breaks the read model.
3. Prove the SSH path live before any UI: a script or a throwaway test that calls `ClaudeSessionLocator.locate` and `readHostFileRange` against a real pane on the mini (`Archive/round22/rpc.py` shows the socket calls). This was never run; the transport methods are covered only by the wire round trip and the CI sshd case.
4. Then 43b as listed under *Build rounds*, with the `resume.md` item-1 text from the round-24 close-out: `ConsoleStore` passthroughs (`paneProcessInfo`, `readHostFileRange`, `sendAgentKeys`), `ChatTranscriptStore`, the list, the setting, the root switch. Decide the iPhone test lane (point 5 above) in the same round.
5. Keep herdr in control: prompts through `agent.prompt`, keys through `agent.send_keys`, the terminal one tap away. The rejected alternatives below were rejected for reasons that still hold.
