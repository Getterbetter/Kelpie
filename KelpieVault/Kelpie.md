...
- Round 16 (2026-09-15, `/delegate`): GhosttyTerminal re-vendored at libghostty-spm `7e45d27` with no patch left, then the branch rebased onto Heeler v0.1.8 (105 commits replayed, 14 files conflicted, three reviews, seven fixes; `HeelerAppModel` owns the stores, one window, taps still land on herdr's screen). Anthony's unit tests passed; installed on both devices, his feature checks next (Open item 33). The pending push went out, the Reddit watch was already running, the composer design (Open item 30) was written and decided, hardware Shift+Tab logged as item 32.
...
- Distribution: App Store 1.0 and the three tips Waiting for Review since 2026-09-12 02:20 UTC as **Kelpie for herdr**; TestFlight public beta live (build 2) at https://testflight.apple.com/join/AkJxAbnJ — see [[App Store plan]]. No pull request upstream for now.
...
| [[Design/Composer text field]] | Open item 30's design: a composing text field mirroring into the PTY, the four decisions taken 2026-09-15 |
