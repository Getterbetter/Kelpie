...
- [ ] **4b. App Store submission.** Anthony's ask (2026-09-11): free app, no TestFlight testing, subreddit for support. Plan, decisions and gates in [[App Store plan]]; local prep done in round 7b.
- [ ] **5. TestFlight upload.** Needs his yes. Now the upload step of [[App Store plan]], not a testing round. `scripts/ExportOptions.plist` already carries team `8JQWBQKEXX`; `make bump && make testflight` is the upstream path.
