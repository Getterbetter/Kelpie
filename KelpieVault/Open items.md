...
- [ ] **12. Key bar on the device.** One keyboard-styled row replaced the chip bar + paste/newline row (`4beeecd`). Left: Anthony's look-and-feel verdict, sticky ctrl then b reaching herdr, Paste with a photo.
...
- [ ] **16. Force-push `kelpie` after the round-11b re-parenting** (`git push --force-with-lease origin kelpie`, Anthony's yes). Round 11 was written into the vault and the stale hashes repaired in round 12; pre-round-7 hashes still resolve only through `kelpie-pre-rebase-20260911`.
- [ ] **17. Load the Reddit watch** (`launchctl bootstrap gui/$(id -u) scripts/launchd/com.kelpie.community watch.plist`, Anthony's yes) and answer the first comment (u/a commenter, r/SideProject, photo resolution over SSH), then `scripts/community watch.py --answered t1_p9bp6zo`. See [[Reddit watch]].
- [ ] **18. Pairing-sync tests on a device.** Seven new tests from round 12 compile but have never run (no device was connected). Run the `PairingSync` suite on the iPad with the next install.
