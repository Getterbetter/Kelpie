...
- [ ] **1. Confirm round 3 on the iPad** (build `77cabda` is installed): Escape and Cmd+. inside herdr (e.g. closing a herdr popup, Escape in Claude Code) · Option+Backspace deleting one word and only one word · Option+Left/Right jumping words · plain Backspace and Return unchanged · the host capsule top-right, and Hosts / Switch Host from it. If Option+Backspace deletes a word *plus* a character, the UIKit echo arrived after the press ended — see the note in `scheduleHardwareKeyClaimReset`.
- [ ] **1b. Decide on onboarding.** Read [[Onboarding proposal]]; two open questions at the bottom are his.
