...
- [ ] **1c. Put the fixed pairing plugin on the mini** (gated on Anthony). The installed plugin is upstream's (`github:ZingerLittleBee/Heeler/plugin@main`); the QR clamp fix lives in this repo's `plugin/`. Either link the local checkout into herdr or upstream the fix.
